...
describe("GET /users/:username", function () {
  test("works for users", async function () {
    await Promise.all([
      User.apply('u1', sampleJobs[0].id),
      User.apply('u1', sampleJobs[2].id)
    ])
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
      user: {
        ...
        email: "user1@user.com",
        isAdmin: false,
        jobs: [sampleJobs[0].id, sampleJobs[2].id]
      },
    });
  });
...
describe("POST /users/:username/jobs/:jobId", function(){
  ...
  test("successful application with correct user", async function() {
    const resp = await request(app)
  ...
  test("BadRequestError on duplicate application", async function() {
    await User.apply('u1', sampleJobs[0].id)
    const resp = await request(app)
      .post(`/users/u1/jobs/${sampleJobs[0].id}`)
      .set("authorization", `Bearer ${u1Token}`)
  ...
  test("NotFoundError on bad job", async function() {
    const resp = await request(app)
  ...
  // test with admin to avoid immediate unauthorized
  ...
  test("Unauthorized on non-admin wrong user", async function() {
    const resp = await request(app)
  ...
  test("Unauthorized on no user", async function() {
    const resp = await request(app)
      .post(`/users/u1/jobs/${sampleJobs[0].id}`)
